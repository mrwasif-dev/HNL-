...
// ============================================================
// BOOTSTRAP — check session, else show ERP ID login
// ============================================================
async function bootstrap() {
    ...
    try {
        ...
        if (!st.configured) {
            document.getElementById('authSubtitle').textContent = 'No ERP IDs configured yet — contact your admin';
        }
    } catch (e) { /* ignore */ }
}
...
document.getElementById('authForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    const erpId = document.getElementById('authErpId').value.trim();
    ...
    try {
        const data = await api('/api/auth/login', { method: 'POST', body: JSON.stringify({ erpId, password }) });
        currentUser = { erpId: data.erpId };
        showDashboard();
    } catch (err) {
...
function showDashboard() {
    ...
    document.getElementById('accountErpId').textContent = currentUser.erpId || '-';
    startAutoRefresh();
    loadFsrConfig();
    initFsrForm();
}
...
function initFsrForm() {
    ...
    // Enter key = tap the ✓ for the "add item" inputs
    document.getElementById('manualItemsInput').addEventListener('keydown', (e) => {
        if (e.key === 'Enter') { e.preventDefault(); confirmAddItem('manualItemsList', 'manualItemsInput'); }
    });
    document.getElementById('serviceItemsInput').addEventListener('keydown', (e) => {
        if (e.key === 'Enter') { e.preventDefault(); confirmAddItem('serviceItemsList', 'serviceItemsInput'); }
    });
}

function confirmAddItem(listId, inputId) {
    ...
    card.innerHTML = `
        <div class="item-card-head">
            <span class="item-card-name">${escapeHtml(name)}</span>
            <div class="item-card-actions">
                <button type="button" class="icon-btn-sm" onclick="editItemCard(this)"><i class="fas fa-pen"></i></button>
                <button type="button" class="icon-btn-sm danger" onclick="this.closest('.item-card').remove()"><i class="fas fa-trash"></i></button>
            </div>
        </div>
        <div class="field" style="margin:10px 0 0;">
            <label>Quantity</label>
            <input type="text" inputmode="numeric" class="item-card-qty" placeholder="e.g. 2">
        </div>
    `;
...
function editItemCard(btn) {
    const card = btn.closest('.item-card');
    const nameHolder = card.querySelector('.item-card-name, .item-card-name-input');

    if (nameHolder.tagName === 'SPAN') {
        const input = document.createElement('input');
        input.type = 'text';
        input.className = 'item-card-name-input';
        input.value = nameHolder.textContent;
        nameHolder.replaceWith(input);
        input.focus();
        btn.innerHTML = '<i class="fas fa-check"></i>';
    } else {
        const span = document.createElement('span');
        span.className = 'item-card-name';
        span.textContent = nameHolder.value.trim() || 'Item';
        nameHolder.replaceWith(span);
...
function collectItems(listId) {
    return Array.from(document.querySelectorAll('#' + listId + ' .item-card')).map(card => {
        const nameHolder = card.querySelector('.item-card-name, .item-card-name-input');
        const name = (nameHolder.tagName === 'SPAN' ? nameHolder.textContent : nameHolder.value).trim();
        const qty = card.querySelector('.item-card-qty').value.trim() || '1';
        return { name, qty };
    }).filter(i => i.name);
}
