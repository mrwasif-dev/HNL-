...
// ---------------------------------------------------------------------------
// DASHBOARD AUTH / PLANS / FORWARD-CONFIG SCHEMAS (global, not per-WA-session)
// ---------------------------------------------------------------------------
const muzammil_dashUserSchema = new mongoose.Schema({
    username: { type: String, required: true, unique: true, trim: true },
    passwordHash: { type: String, required: true },
    createdAt: { type: Date, default: Date.now }
});

// FSR (Field Service Report) config — just the WhatsApp group JID the
// submitted reports get sent to. One per dashboard admin.
const muzammil_fsrConfigSchema = new mongoose.Schema({
    username: { type: String, required: true, unique: true },
    groupJid: { type: String, default: '' }
}, { timestamps: true });

let DashUserModel, FsrConfigModel;
function getDashUserModel() {
    if (!DashUserModel) DashUserModel = mongoose.models.muzammil_DashUser || mongoose.model('muzammil_DashUser', muzammil_dashUserSchema, 'muzammil.dashusers');
    return DashUserModel;
}
...
// ---- Dashboard users ----
async function muzammil_getFirstDashUser() {
    if (!isConnected) return null;
    try { return await getDashUserModel().findOne({}).sort({ createdAt: 1 }); } catch (e) { console.error('DB Error getFirstDashUser:', e); return null; }
}

async function muzammil_countDashUsers() {
    if (!isConnected) return 0;
    try { return await getDashUserModel().countDocuments({}); } catch (e) { console.error('DB Error countDashUsers:', e); return 0; }
}

async function muzammil_createDashUser(username, passwordHash) {
    const Model = getDashUserModel();
    return Model.create({ username, passwordHash });
}

async function muzammil_getDashUserByUsername(username) {
    if (!isConnected) return null;
    try { return await getDashUserModel().findOne({ username }); } catch (e) { console.error('DB Error getDashUserByUsername:', e); return null; }
}

// ---- FSR config ----
async function muzammil_getFsrConfig(username) {
    if (!isConnected) return null;
    try {
        const Model = getFsrConfigModel();
        let cfg = await Model.findOne({ username });
        if (!cfg) cfg = await Model.create({ username });
        return cfg;
    } catch (e) { console.error('DB Error getFsrConfig:', e); return null; }
}

async function muzammil_saveFsrConfig(username, updates) {
    if (!isConnected) return false;
    try {
        await getFsrConfigModel().findOneAndUpdate({ username }, updates, { upsert: true, returnDocument: 'after' });
        return true;
